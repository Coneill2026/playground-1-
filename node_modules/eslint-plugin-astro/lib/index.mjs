...
function requireUserLocal(id) {
	try {
		...
		const module = createRequire(relativeTo)(id);
		if (module && typeof module === "object" && "default" in module && module.default) return module.default;
		return module;
	} catch {
		return null;
	}
...
* Resolve module path
*/
function resolveModule(context, name) {
	try {
...
function transform$3(node, context) {
	if (!resolveModule(context, "postcss-load-config")) return null;
...
function getTransformSync() {
	if (!transformSync) {
		...
		transformSync = createSyncFn(path.join(dirname, "postcss-worker.mjs"));
	}
...
function* extractClassListFromExpression(node, context) {
	...
	if (node.type === AST_NODE_TYPES.ObjectExpression) {
		for (const prop of node.properties) if (prop.type === AST_NODE_TYPES.SpreadElement) yield* extractClassListFromExpression(prop.argument, context);
			...
			if (prop.key.type === AST_NODE_TYPES.Literal) yield String(prop.key.value);
...
function requirePlugin(pluginName) {
	return requireUserLocal(pluginName);
}
/**
...
function getPluginJsxA11y() {
	...
	for (const pluginName of ["eslint-plugin-jsx-a11y", "eslint-plugin-jsx-a11y-x"]) {
...
*/
function getAstroRuleName(ruleName) {
...
function buildFlatConfigs() {
	...
	for (const configName of a11yConfigKeys) Object.defineProperty(configs, `jsx-a11y-${configName}`, {
		...
		get() {
			...
			const newRules = {};
			for (const ruleName of Object.keys(baseRules)) newRules[getAstroRuleName(ruleName)] = baseRules[ruleName];
			return [...base_default, {
				rules: newRules,
				...baseConfig.languageOptions ? { languageOptions: baseConfig.languageOptions } : {}
			}];
		}
	});
	return configs;
...
*/
function getRuleDocsUrl(ruleName, baseRule) {
	if (baseRule?.meta?.docs?.url) return baseRule.meta.docs.url;
...
*/
function getPluginName(baseRule) {
	if ((baseRule?.meta?.docs?.url)?.includes("eslint-plugin-jsx-a11y-x")) return "eslint-plugin-jsx-a11y-x";
